...
data class MealWithItems(val meal: Meal, val items: List<MealItem>)
data class TodayData(val profile: Profile, val localDate: String, val activeGoal: GoalVersion?, val totals: NutrientSnapshot, val hasHistory: Boolean, val meals: List<MealWithItems>, val storeRevision: Long)
data class QuickFood(val food: FoodReference, val isFavorite: Boolean, val lastUsedLocalDate: String? = null)
data class QuickFoods(val favorites: List<QuickFood>, val recents: List<QuickFood>)
sealed interface RepositoryIssue {
    data class Conflict(val operationId: String, val code: String?, val message: String) : RepositoryIssue
    data class MutationRejected(val operationId: String, val message: String) : RepositoryIssue
}
...
interface NutritionRepository {
    ...
    suspend fun loadProfileContext(profileId: String): ProfileContext
    suspend fun updateProfileContext(profileId: String, settings: JsonObject, expectedProfileRevision: Long): ProfileContext
    ...
    suspend fun addFoodToMeal(mealId: String, foodId: String, quantityValue: Double, quantityUnit: String, portionId: String?, expectedMealRevision: Long): AddFoodToMealResponse
    ...
    suspend fun updateMealItemQuantity(itemId: String, quantityValue: Double, quantityUnit: String, portionId: String?, expectedItemRevision: Long, expectedMealRevision: Long): UpdateMealItemResponse
    suspend fun removeMealItem(itemId: String, expectedItemRevision: Long, expectedMealRevision: Long): RemoveMealItemResponse
    suspend fun voidMeal(mealId: String, expectedMealRevision: Long): VoidMealResponse
    suspend fun executeMutation(commandType: String, payload: JsonObject, operationId: String? = null): JsonElement
