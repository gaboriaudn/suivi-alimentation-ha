...
fun ProfileScreen(modifier: Modifier = Modifier, state: ProfileUiState, onSave: (ProfileForm) -> Unit, onReload: () -> Unit, onLogout: () -> Unit) {
    ...
    if (state.loading && context == null) {
        Column(modifier.fillMaxSize(), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
            CircularProgressIndicator(); Text("Chargement du profil…", modifier = Modifier.padding(top = AppSpacing.sm))
        }; return
    }
    if (context == null) {
        Column(modifier.fillMaxSize().padding(AppSpacing.lg), verticalArrangement = Arrangement.spacedBy(AppSpacing.md)) {
            Text("Profil", style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Bold)
            Text(state.error ?: "Profil indisponible", color = MaterialTheme.colorScheme.error)
            Button(onClick = onReload, modifier = Modifier.fillMaxWidth()) { Text("Réessayer") }
        }; return
    }
    ...
    LazyColumn(modifier = modifier.fillMaxSize(), contentPadding = androidx.compose.foundation.layout.PaddingValues(AppSpacing.lg), verticalArrangement = Arrangement.spacedBy(AppSpacing.md)) {
        ...
        item { Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.sm)) {
        ...
        item { Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.sm)) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) { Text("Synchroniser depuis Home Assistant", Modifier.weight(1f)); Switch(form.useHaWeight, { form = form.copy(useHaWeight = it) }) }
            if (form.useHaWeight) {
                ...
                Text(if (resolved.available && resolved.valueKg != null) "Poids actuel : ${resolved.valueKg} kg" else "Poids Home Assistant indisponible", fontWeight = FontWeight.SemiBold)
                resolved.lastUpdated?.let { Text("Dernière mesure : $it", style = MaterialTheme.typography.bodySmall) }
            } else OutlinedTextField(form.manualWeightKg, { form = form.copy(manualWeightKg = it.filter { c -> c.isDigit() || c == '.' || c == ',' }) }, Modifier.fillMaxWidth(), label = { Text("Poids actuel (kg)") }, singleLine = true)
        } } }
        item { Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.sm)) {
            ...
            if (form.automaticGoals) {
                ...
                if (rec != null) { Text("Proposition : ${rec.energyKcal?.toInt()} kcal · ${rec.proteinG?.toInt()} g de protéines", fontWeight = FontWeight.SemiBold); Text("Calcul basé sur le profil, le poids actuel et le niveau d’activité.", style = MaterialTheme.typography.bodySmall) }
        ...
        item { Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.sm)) {
            ...
            OutlinedButton(onClick = onLogout, modifier = Modifier.fillMaxWidth().heightIn(min = MinimumTouchTarget)) { Text("Se déconnecter de Home Assistant") }
        } } }
...
@Composable private fun ChoiceField(label: String, value: String, options: List<Pair<String, String>>, onSelect: (String) -> Unit) {
    var expanded by remember { mutableStateOf(false) }; Column { Text(label, style = MaterialTheme.typography.labelLarge); OutlinedButton({ expanded = true }, Modifier.fillMaxWidth()) { Text(options.firstOrNull { it.first == value }?.second ?: "Choisir") }; DropdownMenu(expanded, { expanded = false }) { options.forEach { option -> DropdownMenuItem({ Text(option.second) }, { expanded = false; onSelect(option.first) }) } } }
}