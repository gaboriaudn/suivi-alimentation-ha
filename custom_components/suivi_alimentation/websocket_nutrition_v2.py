...
def async_setup(hass: HomeAssistant) -> None:
    ...
    websocket_api.async_register_command(hass, create_manual_food)
...
@websocket_api.websocket_command({
    "type": f"{DOMAIN}/v2/search_personal_foods", "profile_id": str, "query": str,
...
async def search_personal_foods(hass, connection, msg) -> None:
    ...
    if repository is None or nutrition is None or not _profile_access(connection, repository, profile_id):
        ...
        return
    try:
        items = nutrition.search_personal_foods(_personal_foods(hass, profile_id), msg["query"], _existing_personal_by_legacy_id(repository, profile_id), int(msg.get("limit", 20)))
...
@websocket_api.websocket_command({
    "type": f"{DOMAIN}/v2/create_manual_food", "profile_id": str, "label": str,
    "nutrients_per_100g": dict, "operation_id": str,
})
@websocket_api.async_response
async def create_manual_food(hass, connection, msg) -> None:
    ...
    source = {
        "sourceType": "manual",
        "sourceExternalId": str(uuid.uuid4()),
        "sourceVersion": "manual-v1",
        "label": label,
        "nutritionBasis": "per_100g",
        "nutrientsPer100g": msg["nutrients_per_100g"],
        "servingDefinitions": [],
    }
...
@websocket_api.websocket_command({"type": f"{DOMAIN}/v2/import_personal_food", "profile_id": str, "legacy_food_id": str, "operation_id": str})
@websocket_api.async_response
async def import_personal_food(hass, connection, msg) -> None:
    repository, nutrition, _ = _services(hass); profile_id = msg["profile_id"]
    if repository is None or nutrition is None or not _profile_access(connection, repository, profile_id):
        connection.send_error(msg["id"], "unauthorized", "Profile unavailable"); return
    try:
        legacy_id = msg["legacy_food_id"].strip()
        legacy_food = next((food for food in _personal_foods(hass, profile_id) if str(food.get("id") or "") == legacy_id), None)
        if legacy_food is None:
            connection.send_error(msg["id"], "not_found", "Personal food not found"); return
        await _import_source(hass, connection, msg, nutrition.personal_source(legacy_food))
    except Exception as err: _error(connection, msg["id"], err)
...
async def import_off_food(hass, connection, msg) -> None:
    ...
    try:
        ...
        if source is None:
            connection.send_error(msg["id"], "not_found", "Product not found"); return
        await _import_source(hass, connection, msg, source)
    except Exception as err: _error(connection, msg["id"], err)


@websocket_api.websocket_command({"type": f"{DOMAIN}/v2/add_food_to_meal", "meal_id": str, "food_id": str, "quantity_value": vol.Coerce(float), "quantity_unit": str, vol.Optional("portion_id"): str, "expected_meal_revision": vol.Coerce(int), "operation_id": str})
@websocket_api.async_response
async def add_food_to_meal(hass, connection, msg) -> None:
    repository, _, commands = _services(hass)
    if repository is None or commands is None:
        connection.send_error(msg["id"], "not_ready", "Nutrition v2 not initialized"); return
    try:
        result = await commands.async_add_food_to_meal(meal_id=msg["meal_id"], food_id=msg["food_id"], quantity_value=msg["quantity_value"], quantity_unit=msg["quantity_unit"], portion_id=msg.get("portion_id"), operation_id=msg["operation_id"], expected_meal_revision=msg["expected_meal_revision"])
        connection.send_result(msg["id"], result)
    except Exception as err: _error(connection, msg["id"], err)


@websocket_api.websocket_command({"type": f"{DOMAIN}/v2/update_food_meal_item", "item_id": str, "quantity_value": vol.Coerce(float), "quantity_unit": str, vol.Optional("portion_id"): str, "expected_item_revision": vol.Coerce(int), "expected_meal_revision": vol.Coerce(int), "operation_id": str})
@websocket_api.async_response
async def update_food_meal_item(hass, connection, msg) -> None:
    _, _, commands = _services(hass)
    if commands is None:
        connection.send_error(msg["id"], "not_ready", "Nutrition v2 not initialized"); return
    try:
        result = await commands.async_update_food_meal_item(item_id=msg["item_id"], quantity_value=msg["quantity_value"], quantity_unit=msg["quantity_unit"], portion_id=msg.get("portion_id"), operation_id=msg["operation_id"], expected_item_revision=msg["expected_item_revision"], expected_meal_revision=msg["expected_meal_revision"])
        connection.send_result(msg["id"], result)
    except Exception as err: _error(connection, msg["id"], err)
